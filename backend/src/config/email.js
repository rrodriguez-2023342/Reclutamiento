...
// Valida que la configuracio SMTP tenga los campos necesarios para enviar correos
const validateEmailConfig = (config) => {
  const missing = ["host", "user", "pass"].filter((key) => !config[key]);
  if (missing.length) {
    throw new Error(
      `Configuración SMTP incompleta: falta ${missing.join(", ")}`,
    );
  }
};
...
const CONTENIDO_ESTADO = {
  ...
  CONTRATADO: {
    ...
    mensaje:
      "¡Excelentes noticias! Después de revisar tu perfil, hemos decidido contratarte. En los próximos días nos pondremos en contacto contigo para coordinar tu incorporación y los detalles administrativos.",
  },
  RECHAZADO: {
    ...
    mensaje:
      "Agradecemos el interés y el tiempo dedicado a tu postulación. Luego de analizarla cuidadosamente, en esta ocasión no podremos continuar con tu proceso. Te invitamos a volver a postularte en el futuro.",
  },
  POSTULANTE: {
    ...
    mensaje:
      "Te informamos que tu postulación fue reactivada y vuelve al estado inicial del proceso. Tu información seguirá siendo considerada y te notificaremos ante cualquier avance.",
  },
};
