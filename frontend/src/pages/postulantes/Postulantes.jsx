...
const statusLabels = Object.fromEntries(STATUS_OPTIONS.slice(1).map(({ value, label }) => [value, label]))

function formatDpi(dpi = '') {
  const digits = String(dpi).replace(/\D/g, '')
  return digits.length === 13 ? `${digits.slice(0, 4)}-${digits.slice(4, 9)}-${digits.slice(9)}` : dpi
}
...
function SelectField({ ariaLabel, value, onChange, children }) {
  return <div className="relative"><select aria-label={ariaLabel} value={value} onChange={(event) => onChange(event.target.value)} className="h-14 w-full appearance-none rounded-2xl border border-[#dce3ee] bg-white px-4 pr-10 text-base font-semibold text-[#071b3b] outline-none transition focus:border-[#3162e9] focus:ring-2 focus:ring-[#3162e9]/15">{children}</select><ChevronDown aria-hidden="true" className="pointer-events-none absolute right-4 top-1/2 h-5 w-5 -translate-y-1/2 text-[#65758f]" /></div>
}
...
function Postulantes() {
  ...
  useEffect(() => {
    let active = true
    getPlazasPostulantes().then((data) => active && setPlazas(data)).catch(() => active && setPlazas([]))
    return () => { active = false }
  }, [])

  useEffect(() => {
    let active = true
    getPostulantes({ page, limit: PAGE_SIZE, ...(debouncedSearch && { q: debouncedSearch }), ...(estado && { estado }), ...(puesto && { puesto }) })
      .then((data) => { if (active) { setResult(data); setError('') } })
      .catch((requestError) => {
        if (active) setError(requestError.response?.data?.message || 'No fue posible cargar los postulantes.')
      })
  ...
  const updateSearch = (value) => { setSearch(value); setPage(1); setLoading(true) }
  const updateEstado = (value) => { setEstado(value); setPage(1); setLoading(true) }
  const updatePuesto = (value) => { setPuesto(value); setPage(1); setLoading(true) }
  const goToPage = (nextPage) => { setPage(nextPage); setLoading(true) }
      ...
      <section className="mt-7 overflow-hidden rounded-[26px] bg-white shadow-[0_10px_24px_rgba(20,43,89,0.06)]">
        <div className="overflow-x-auto">
          <table className="w-full min-w-[980px] border-separate border-spacing-0 text-left">
            <thead><tr className="text-base font-medium text-[#5b6e8b]"><th className="border-b border-[#dfe5ee] px-7 py-5 font-medium">Nombre Completo</th><th className="border-b border-[#dfe5ee] px-5 py-5 font-medium">DPI (Guatemala)</th><th className="border-b border-[#dfe5ee] px-5 py-5 font-medium">Plaza Aplicada</th><th className="border-b border-[#dfe5ee] px-5 py-5 font-medium">Estado</th><th className="border-b border-[#dfe5ee] px-5 py-5 font-medium">Registro</th><th className="border-b border-[#dfe5ee] px-7 py-5 text-right font-medium">Acciones</th></tr></thead>
            <tbody>
